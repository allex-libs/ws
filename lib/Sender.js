...
Sender.prototype.handleError = function (cb, error) {
...
Sender.prototype.close = function(code, data, mask, cb) {
  ...
  code = code || 1000;
  var dataBuffer = Buffer.alloc(2 + (data ? Buffer.byteLength(data) : 0));
...
Sender.prototype.frameAndSend = function(opcode, data, finalFragment, maskData, compressed, cb) {
  ...
  if (!data) {
    try {
      this._socket.write(Buffer.from([opcode | (finalFragment ? 0x80 : 0), 0 | (maskData ? 0x80 : 0)].concat(maskData ? [0, 0, 0, 0] : [])), 'binary', cb);
    }
    catch (e) {
      this.handleError(cb, e);
    }
    return;
  }
  ...
  var outputBuffer = Buffer.alloc(totalLength);
  ...
  if (maskData) {
    ...
    if (mergeBuffers) {
      ...
      try {
        if (this._socket.writable == false) {
          this.handleError(cb, new Error('Socket not writable'));
          return;
        }
